...
function WatchHistory() {
  ...
  const playVideo = async (movieId) => {
    try {
      ...
      if (youtubeVideo) setVideo(youtubeVideo.key)
      else alert("No video available for this movie")
    } catch (err) {
      ...
      <div className='grid grid-cols-2 md:grid-cols-4 gap-6 p-8'>
        {history.map(movie => (
          ...
          >
            ...
            <button
              onClick={(e) => {
                // Prevent triggering playVideo
                e.stopPropagation()
                const confirmRemove = window.confirm("Are you sure you want to remove this movie?")
                if (confirmRemove) dispatch(RemoveFromHistory(movie.id))
              }}
            ...
            <img
              src={`${IMAGE_URL}${movie.poster || movie.poster_path}`}
