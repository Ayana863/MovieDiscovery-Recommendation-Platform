...
function Comedy() {
  ...
  useEffect(() => {
    const ComedyMovies = async () => {
      try {
        const response = await axios.get(`${DISCOVER_MOVIES_URL}&with_genres=35`)
  ...
  // Search filter 
  const filteredComedyMovies = comedy.filter(movie =>
  ...
  return (
    <>
      ...
      {filteredComedyMovies.length === 0 && searchValue && (
